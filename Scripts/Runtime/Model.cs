...
namespace FullCircleData
{
    ...
    public abstract class Model : BestBehaviour
    {
        ...
        protected override void OnEnable()
        {
            ...
            Invoke(nameof(EnableBetterBehaviour), 0);
        }

        private void EnableBetterBehaviour()
        {
            base.OnEnable();   
        }
        ...
        public static Observable<T> Find<T>(Transform context, string fieldName, bool skipTypeCheck = false)
        {
            ...
            var source = context.GetComponentInParent<Model>();
            ...
            if (observable == null)
            {
                var models = source.transform.GetComponents<Model>();
                foreach (var model in models)
                {
                    if (model == source) continue;
                    observable = model.GetObservable<T>(fieldName, skipTypeCheck);
